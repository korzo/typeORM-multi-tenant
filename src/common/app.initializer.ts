 ...
 private async configurationDatabaseTenants() {
  ...
  try {
   ...
   for (const tenant of list) {
    ...
    let tenantConfiguration: TenantConfiguration[] = await connection.query(`SELECT * FROM tcf.tenant_configuration where id_tenant = '${tenant.id}'`)
    let configurationItem: configurationItem[] = await connection.query(`SELECT * FROM tcf.configuration_item where id_config_type = '${tenantConfiguration[0].id_config_type_database}'`)
